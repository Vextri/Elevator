#include "../include/pcanFunctions.h"

#include <stdio.h>
#include <stdlib.h>
#include <stdlib.h>  
#include <errno.h>
#include <unistd.h> 
#include <signal.h>
#include <string.h>
#include <fcntl.h>    					// O_RDWR
#include <unistd.h>
#include <ctype.h>
#include <libpcan.h>   					// PCAN library


// Globals
// ***********************************************************************************************************
HANDLE h;
HANDLE h2;
TPCANMsg Txmsg;
TPCANMsg Rxmsg;
DWORD status;

// Code
// ***********************************************************************************************************

// Functions
// *****************************************************************
int pcanTx(int id, int data){
	h = LINUX_CAN_Open("/dev/pcanusb32", O_RDWR);		// Open PCAN channel

	// Initialize an opened CAN 2.0 channel with a 125kbps bitrate, accepting standard frames
	status = CAN_Init(h, CAN_BAUD_125K, CAN_INIT_TYPE_ST);

	// Clear the channel - new - Must clear the channel before Tx/Rx
	status = CAN_Status(h);

	// Set up message
	Txmsg.ID = id; 	
	Txmsg.MSGTYPE = MSGTYPE_STANDARD; 
	Txmsg.LEN = 1; 
	Txmsg.DATA[0] = data; 

	sleep(1);  
	status = CAN_Write(h, &Txmsg);

	// Close CAN 2.0 channel and exit	
	CAN_Close(h);
}

int pcanRx(int num_msgs){
	int i = 0;
	u_int16_t value;
	// Open a CAN channel 
	h2 = LINUX_CAN_Open("/dev/pcanusb32", O_RDWR);

	// Initialize an opened CAN 2.0 channel with a 125kbps bitrate, accepting standard frames
	status = CAN_Init(h2, CAN_BAUD_125K, CAN_INIT_TYPE_ST);

	// Clear the channel - new - Must clear the channel before Tx/Rx
	status = CAN_Status(h2);

	// Clear screen to show received messages
	//system("@cls||clear");

	// receive CAN message  - CODE adapted from PCAN BASIC C++ examples pcanread.cpp
	printf("\nReady to receive message(s) over CAN bus\n");
	
	// Read 'num' messages on the CAN bus
	while(i < num_msgs) {
		while((status = CAN_Read(h2, &Rxmsg)) == PCAN_RECEIVE_QUEUE_EMPTY){
			sleep(1);
		}
		if(status != PCAN_NO_ERROR) {						// If there is an error, display the code
			printf("Error 0x%x\n", (int)status);
			//break;
		}
		
	
												
		if(Rxmsg.ID != 0x01 && Rxmsg.LEN >= 2) {
			value = ((u_int16_t)Rxmsg.DATA[1] << 8) | Rxmsg.DATA[0];
			printf("  - R ID:%4x LEN:%1x DATA: %02x %02x %02x => Value: %d\n",
				   (int)Rxmsg.ID, 
				   (int)Rxmsg.LEN,
				   (int)Rxmsg.DATA[2], 
				   (int)Rxmsg.DATA[1],
				   (int)Rxmsg.DATA[0], 
				   value);
			
		}
		else if (Rxmsg.ID != 0x01 && Rxmsg.LEN == 1){
			value = (u_int16_t)Rxmsg.DATA[0];
			printf("  - R ID:%4x LEN:%1x DATA: %02x => Value: %d\n",
               (int)Rxmsg.ID,
               (int)Rxmsg.LEN,
               (int)Rxmsg.DATA[0],
               value);
               
		}
		i++;

	}
	

	// Close CAN 2.0 channel and exit	
	CAN_Close(h2);
	//printf("\nEnd Rx\n");
	return ((int)Rxmsg.DATA[0]);						// Return the last value received
}


int pCanRx_db()
{
	int i = 0;
	u_int16_t value;
	// Open a CAN channel 
	h2 = LINUX_CAN_Open("/dev/pcanusb32", O_RDWR);

	// Initialize an opened CAN 2.0 channel with a 125kbps bitrate, accepting standard frames
	status = CAN_Init(h2, CAN_BAUD_125K, CAN_INIT_TYPE_ST);

	// Clear the channel - new - Must clear the channel before Tx/Rx
	status = CAN_Status(h2);

	// Clear screen to show received messages
	//system("@cls||clear");

	// receive CAN message  - CODE adapted from PCAN BASIC C++ examples pcanread.cpp
	printf("\nReady to receive message(s) over CAN bus\n");
	
	// Read 'num' messages on the CAN bus
	while(i == 0) {
		while((status = CAN_Read(h2, &Rxmsg)) == PCAN_RECEIVE_QUEUE_EMPTY){
			sleep(1);
		}
		if(status != PCAN_NO_ERROR) {						// If there is an error, display the code
			printf("Error 0x%x\n", (int)status);
			//break;
		}
		
		
		if (Rxmsg.ID != 0x01 && Rxmsg.LEN == 1){
			value = (u_int16_t)Rxmsg.DATA[0];
			printf("  - R ID:%4x LEN:%1x DATA: %02x => Value: %d\n",
               (int)Rxmsg.ID,
               (int)Rxmsg.LEN,
               (int)Rxmsg.DATA[0],
               value);
            if ( Rxmsg.ID == ID_CC_TO_SC|| Rxmsg.ID == ID_F1_TO_SC || Rxmsg.ID == ID_F2_TO_SC || Rxmsg.ID == ID_F3_TO_SC)
            {
				pcanTx(ID_SC_TO_EC,(int)Rxmsg.DATA[0]);
			}
			
			i=1;
            break;
		}
        else if(Rxmsg.ID != 0x01 && Rxmsg.LEN >= 2) {
			value = ((u_int16_t)Rxmsg.DATA[1] << 8) | Rxmsg.DATA[0];
			printf("  - R ID:%4x LEN:%1x DATA: %02x %02x %02x => Value: %d\n",
				   (int)Rxmsg.ID, 
				   (int)Rxmsg.LEN,
				   (int)Rxmsg.DATA[2], 
				   (int)Rxmsg.DATA[1],
				   (int)Rxmsg.DATA[0], 
				   value);
			i=1;
			break;
			
		}      

	}
	

	// Close CAN 2.0 channel and exit	
	CAN_Close(h2);
	//printf("\nEnd Rx\n");
	return (value);						// Return the last value received
}



