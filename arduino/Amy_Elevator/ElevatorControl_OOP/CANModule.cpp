...
#include "CANModule.h"
#include "audio.h"                                         // Include audio helper functions to play audio files for each floor
...
uint32_t CANModule::getTxdata()                               // Get the first byte of data from txdata array (per our protocol) - modify if you want to use more than first byte
{
  return txdata;
}  
                       
void CANModule::setTxdata(byte data, uint16_t setpoint)                                      // Set the first byte of data from txdata array (per our protocol) - modify if you want to use more than first byte  
{
  txdata[0] = data;
...
void CANModule::receiveCAN(LCD lcd) {
    ...
    // Include 2 bytes for distance measurement
