...
#define DLC 4                               // Data length code in CAN (we only use one of the possible 8 bytes). This code can handle DLC from 1 to 8. 
...
public:
  ...
  uint32_t getTxdata();                         // Get the first 4 bytes of data from txdata array (per our protocol) - modify if you want to use more than first byte
  void setTxdata(byte, uint16_t);                     // Set the first 4 bytes of data from txdata array (per our protocol) - modify if you want to use more than first byte 
